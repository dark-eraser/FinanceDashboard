...
def classify_merchants_with_hybrid(csv_file, merchant_col=None):
    # Preprocessed files always use comma delimiter
    # Only use semicolon for raw/original ZKB files
    filename = os.path.basename(csv_file).lower()
    if "zkb" in filename and "preprocessed" not in filename:
    ...
    # Find merchant column if not specified
    if merchant_col is None or merchant_col not in df.columns:
        for fallback in [
            "description",
            "Description",
            "Booking text",
    ...
    # Find date column
    date_col = None
    for fallback in ["value_date", "Date", "date", "Completed Date", "Started Date"]:
        if fallback in df.columns:
            date_col = fallback
            break
    ...
    # Find amount column
    amount_col = None
    for fallback in ["amount", "Amount", "Debit CHF", "Credit CHF"]:
        if fallback in df.columns:
            amount_col = fallback
            break

    # If still no merchant column found, raise error
    if merchant_col is None or merchant_col not in df.columns:
        print(f"Error: No merchant column found in {csv_file}")
    ...
    for merchant in df[merchant_col].dropna().unique():
        ...
        merchant_transactions = df[df[merchant_col] == merchant]

        while True:
            print(f"\n{'='*80}")
            print(f"Merchant: {merchant}")
            print(f"Number of transactions: {len(merchant_transactions)}")

            # Show sample transactions with date and amount
            print("\nSample transactions:")
            for idx, (_, row) in enumerate(merchant_transactions.head(5).iterrows()):
                date_str = (
                    row[date_col] if date_col and date_col in row.index else "N/A"
                )
                amount_str = (
                    row[amount_col] if amount_col and amount_col in row.index else "N/A"
                )
                print(f"  {idx+1}. Date: {date_str}, Amount: {amount_str}")

            if len(merchant_transactions) > 5:
                print(f"  ... and {len(merchant_transactions) - 5} more transactions")
